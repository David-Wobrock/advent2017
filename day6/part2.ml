...
let all_items: int list list ref = ref [(Array.to_list items)] in
    let counter = ref 1 in
        ...
        let seen_state = Array.copy items in
            let counter2 = ref 1 in
                step items;
                while not (arrays_are_equal (Array.to_list seen_state) (Array.to_list items)) do
                    step items;
                    counter2 := (!counter2 + 1);
                done;
        print_endline (string_of_int (!counter2));
        ;;
